...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // Helpers de badge
  function showUpdateBadge(text = '!') {
    if (!btnUpdate.querySelector('.button-badge')) {
  ...
  function removeUpdateBadge() {
    const b = btnUpdate.querySelector('.button-badge');
    if (b) b.remove();
  }

  // Primeiro clique dispara a checagem
  btnUpdate.onclick = () => {
    removeUpdateBadge();
  ...
  window.api.onUpdateAvailable(() => {
    showTempAlert('🌟 Update disponível!', 4000);
  ...
  window.api.onUpdateNotAvailable(() => {
    showTempAlert('✅ App atualizado', 3000);
    btnUpdate.textContent = 'Verificar Update';
    btnUpdate.disabled    = false;
    removeUpdateBadge();
  });
  window.api.onError(err => {
    showTempAlert(`❌ Erro: ${err}`, 4000);
    btnUpdate.textContent = 'Verificar Update';
    btnUpdate.disabled    = false;
    removeUpdateBadge();
  });
  ...
  window.api.onProgress(({ processed, total, entry }) => {
    ...
    if (processed === 1) {
      ulNo.innerHTML        = '';
      ulYes.innerHTML       = '';
      btnDelControls.style.display = 'none';
    }
    ...
    if (!isRootEmpty) {
      ...
      if (entry.files.length > 0) {
        ...
        btnDelControls.style.display = 'inline-block';
      } else {
        ulNo.appendChild(li);
      }
    }
